...
function _load_data_from_file() {
    ...
    if [[ "$DATA_FORMAT" == "json" ]]; then
        ...
        # Load from CSV file
        # Expected format: url,source_server_ip,source_ssh_user,dest_site_id,dest_server_id,dest_server_label,dest_server_ip,dest_system_user_id,dest_system_user_name,dest_ssh_user
        local csv_line
        ...
        IFS=',' read -r source_site_url source_server_ip source_ssh_user_csv dest_site_id dest_server_id dest_server_label dest_server_ip dest_system_user_id dest_system_user_name dest_ssh_user_csv <<< "$csv_line"
...
EOF
    ...
    IFS='|' read -r source_db source_db_user source_db_pass <<< "$source_tuple"
    if [[ -z "$source_db" ]]; then
        if [[ "$DEBUG" == "1" ]]; then
...
EOF
    ...
    IFS='|' read -r dest_db _ _ <<< "$dest_tuple"
    if [[ -z "$dest_db" ]]; then
        if [[ "$DEBUG" == "1" ]]; then
            local dbg_cmd dbg_out
            dbg_cmd=$(cat <<'EOF'
...
function _step_3_4() {
    ...
    _loading2 "Source: $ssh_user@$source_server_ip:$source_site_path/"
    ...
    local can_reach
    can_reach=$(_ssh_capture "$dest_server_ip" "ssh -o ConnectTimeout=5 -o BatchMode=yes -o StrictHostKeyChecking=accept-new -p $source_ssh_port $ssh_user@$source_server_ip 'echo ok' 2>/dev/null || echo 'CANNOT_REACH'")
    ...
    if [[ "$can_reach" == "ok" ]]; then
        ...
        remote_rsync_cmd="rsync $rsync_opts -e 'ssh -o BatchMode=yes -o StrictHostKeyChecking=accept-new -p $source_ssh_port' $ssh_user@$source_server_ip:$source_site_path/ $dest_htdocs_path/"
        ...
        local pull_cmd
        pull_cmd="rsync $rsync_opts -e \"$ssh_opts -p $source_ssh_port\" $ssh_user@$source_server_ip:$source_site_path/ $tmp_dir/"
...
# Uses route captured in Step 1.2 and applies it to destination via:
...
function _step_7() {
    ...
    if [[ "$source_is_ssl" != "true" ]]; then
        ...
        _log "STEP 7: Source SSL not enabled, skipping"
        _state_write ".data.ssl_enabled" "false"
        _state_add_completed_step "7"
        _success "Step 7 complete: SSL not required (source has no SSL)"
        echo
        return 0
    fi
    ...
    if [[ -z "$dest_domain_id" || "$dest_domain_id" == "null" ]]; then
        ...
        _log "STEP 7 FAILED: dest_domain_id not in state"
        return 1
    fi
