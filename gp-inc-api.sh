...
# -- Returns domain URLs, one per line
# ======================================
...
function _gp_api_get_domain () {
    ...
    # Match by url, domain_url, or name field
    local result
    result=$(jq --arg url "$DOMAIN_URL" '.[] | select(.url == $url or .domain_url == $url or .name == $url)' "$CACHE_FILE" 2>/dev/null)
...
function _gp_api_get_domains_for_site () {
    ...
    jq --arg sid "$SITE_ID" '[ .[] | select(.site_id == ($sid | tonumber)) ]' "$CACHE_FILE" 2>/dev/null
    return 0
}
...
function _gp_api_get_primary_domain_for_site () {
    ...
    local primary
    primary=$(jq -r --arg sid "$SITE_ID" '.[] | select(.site_id == ($sid | tonumber) and .type == "primary") | .url // .domain_url // .name' "$CACHE_FILE" 2>/dev/null | head -1)
    ...
    local alias_domain
    alias_domain=$(jq -r --arg sid "$SITE_ID" '.[] | select(.site_id == ($sid | tonumber)) | .url // .domain_url // .name' "$CACHE_FILE" 2>/dev/null | head -1)
